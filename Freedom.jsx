...
const Freedom = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await axios.get("http://localhost:9091/mobile-plans/freedom");
        setFreedomPlans(response.data);
        setLoading(false);
      } catch (error) {
        setError("An error occurred while fetching Freedom plans. Please try again later.");
        setLoading(false);
